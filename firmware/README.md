...
Zmiany:
1. Możliwość ustawiania WPM RX(MIN MAX), FWPM/BWPM RX(MIN MAX), TONE RX(MIN MAX) i VOLUMERX (MIN MAX) na wejściu ćwiczeń Head Copy i  Copy Callisign. Wartość jest losowane pomiędzy MIN MAX. Żeby zachować realizm zmienia się dopier w następnej próbie (AGN lub błędna próba nie zmienia wartości)
